...
const CategoryPieChart: React.FC<ChartProps> = ({ filters }) => {
  const { data, isLoading, isError } = useGetCategoryBreakdownQuery({ ...filters, type: 'expense' });

  if (isLoading) {
    return <Box display="flex" justifyContent="center" alignItems="center" height={200}><CircularProgress /></Box>;
  }
